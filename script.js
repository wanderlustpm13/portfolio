(function () {
  ...
  (function heroReveal() {
    const hero = document.querySelector(".hero__text");
    const gsap = window.gsap;
    if (!hero || !gsap || typeof window.SplitText === "undefined") return;

    // Respect reduced-motion: leave the hero fully visible, no animation.
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;

    // Hide immediately to avoid a flash before the reveal runs.
    ...
    const run = () => {
      ...
      try {
        ...
        // Split by words only: whitespace splits, so each word keeps its
        // inline glyph span (pencil/heart) and <em> accent intact.
        const split = new window.SplitText(hero, {
          type: "words",
          wordsClass: "hero__word",
        });
        const rand = gsap.utils.random;
        // Capture the names' normal colour before we override it to yellow.
        const ems = hero.querySelectorAll("em");
        const origColor = ems.length ? getComputedStyle(ems[0]).color : null;

        const tl = gsap.timeline({ delay: 0.15 });
        tl.set(hero, { autoAlpha: 1 });
        // Names start brand yellow so they're highlighted as they FLY IN.
        if (ems.length) tl.set(ems, { color: "#FFDD2D" });
        // Words fly in from random offsets + rotation and overshoot into place,
        // resolving in random order for a lively "assemble" feel.
        tl.from(split.words, {
          opacity: 0,
          x: () => rand(-180, 180),
          y: () => rand(-90, 90),
        ...
        // Hold the yellow a bit after everything settles, then fade to normal.
        if (ems.length) {
          tl.to(
            ems,
            {
            ...
            "+=1.4" // hold after settling before fading back
          );
        }
      } catch (err) {
    ...
    // Fallbacks so the hero never stays hidden if fonts.ready stalls.
